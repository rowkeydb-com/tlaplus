 ...
 *  A TLA+ Model checker
 */
// SZ Feb 20, 2009: major refactoring of this class introduced due to the
// changes in the type hierarchy. multiple methods has been pulled up to the
// super class. unused constructors has been removed the class now contains only
// the parts, which are different from the DFIDModelChecker the name resolver
// and support for the external specification object has been added
public class ModelChecker extends AbstractChecker {

  protected static final boolean ActionCoverage =
      TLCGlobals.Coverage.isActionEnabled();
  /**
   * If the state/ dir should be cleaned up after a successful model run
   */
  public static final boolean VETO_CLEANUP =
      Boolean.getBoolean(ModelChecker.class.getName() + ".vetoCleanup");
  ...
  /* Constructors  */
  public ModelChecker(ITool tool, String metadir,
                      final IStateWriter stateWriter, boolean deadlock,
                      String fromChkpt, final Future<FPSet> future,
    ...
    for (int i = 0; i < this.workers.length; i++) {
      this.workers[i] = this.trace.addWorker(
          new Worker(i, this, i == 0 ? tool : tool.noDebug(), this.metadir,
                     this.tool.getRootName()));
    }
  }

  public ModelChecker(ITool tool, String metadir,
                      final IStateWriter stateWriter, boolean deadlock,
                      String fromChkpt, final FPSetConfiguration fpSetConfig,
                      long startTime) throws EvalException, IOException {
    this(tool, metadir, stateWriter, deadlock, fromChkpt, startTime);
    this.theFPSet =
        FPSetFactory.getFPSet(fpSetConfig)
            .init(TLCGlobals.getNumWorkers(), metadir, tool.getRootName());
   ...
   * SZ Feb 20, 2009
   * @param resolver name resolver to be able to load files (specs and configs)
   *     from managed environments
   * @param specObj external SpecObj added to enable to work on existing
   *     specification
   */
  private ModelChecker(ITool tool, String metadir,
                       final IStateWriter stateWriter, boolean deadlock,
                       String fromChkpt, long startTime)
    ...
    this.trace = new ConcurrentTLCTrace(this.metadir, this.tool.getRootName(),
                                        this.tool);
  }
    ...
    if (!recovered) {
      ...
      try {
        ...
        if (result != EC.NO_ERROR) {
          ...
          if (this.errState != null) {
            tool.checkPostConditionWithCounterExample(
                new CounterExample(errState));
          } else {
        ...
        /**
         * The following code replaced code equivalent to setting msg =
         * e.getMessage(). getMessage() for a StackOverflowError returned null,
         * producing a useless error message.  Changed by LL on 12 Mar 2010
         */
        if (e instanceof StackOverflowError) {
          msg =
              "This was a Java StackOverflowError. It was probably the result\n"
              + "of an incorrect recursive function definition that caused " +
                "TLC to enter\n"
              + "an infinite loop when trying to compute the function or its " +
                "application\n"
        ...
        if (this.errState != null) {
          MP.printError(EC.TLC_INITIAL_STATE,
                        new String[] {msg, this.errState.toString()});
        } else {
        ...
        try {
          ...
          result = MP.printError(EC.TLC_FINGERPRINT_EXCEPTION,
                                 new String[] {cTool.hasCallStack()
                                                   ? cTool.toString()
                                                   : fe.getTrace(),
                                               fe.getRootCause().getMessage()});
        } catch (Throwable e1) {
          // Assert.printStack(e);
      ...
      if (statesGenerated == this.theFPSet.size()) {
        MP.printMessage(EC.TLC_INIT_GENERATED1,
                        new String[] {String.valueOf(statesGenerated), plural});
      } else {
        MP.printMessage(EC.TLC_INIT_GENERATED2,
                        new String[] {String.valueOf(statesGenerated), plural,
    ...
    try {
      ...
      if (this.errState == null) {
        ...
        if (this.checkLiveness) {
          ...
          // checking is.
          MP.printMessage(
              EC.TLC_PROGRESS_STATS,
              new String[] {String.valueOf(this.trace.getLevelForReporting()),
        ...
        final CallStackTool cTool = new CallStackTool(this.tool);
        try {
          // Not adding newly created Worker to trace#addWorker because it is
          // not supposed to rewrite the trace file but to reconstruct actual
          // states referenced by their fingerprints in the trace.
          this.doNext(
              cTool, this.predErrState,
          ...
          // Do not replace the actual error code, such as assert violation,
          // with TLC_NESTED_EXPRESSION.
      ...
      if (this.checkLiveness) {
        if (LIVENESS_STATS) {
          ...
          MP.printStats(
              liveCheck.calculateInDegreeDiskGraphs(
                  new BucketStatistics("Histogram vertex in-degree",
                                       LiveCheck.class.getPackage().getName(),
                                       "DiskGraphsInDegree")),
   ...
   */
  public int checkAssumptions() { return this.tool.checkAssumptions(); }
  ...
  private final int doInit(final ITool tool, boolean ignoreCancel)
      throws Throwable {
    ...
    if (ignoreCancel) {
      // Rerunning state space exploration to reconstruct the error stack to
      // determine what caused Tool to fail to evaluate the init predicate
      // expressions. Thus, re-check all invariants even if state is already
      // known (= part of theFPSet).
   ...
   */
  private final boolean doNext(final ITool tool, TLCState curState,
                               final SetOfStates liveNextStates,
                               final Worker worker) throws Throwable {
    ...
    try {
      for (int i = 0; i < tool.getActions().length; i++) {
        ...
        for (int j = 0; j < sz; j++) {
          ...
          final boolean inModel = (tool.isInModel(succState) &&
                                   tool.isInActions(curState, succState));
          boolean unseen = true;
          if (inModel) {
            unseen = !isSeenState(curState, succState, action, worker,
                                  liveNextStates);
          }
      ...
      if (deadLocked && this.checkDeadlock) {
        return doNextSetErr(curState, null, false, EC.TLC_DEADLOCK_REACHED,
                            null);
      }
      return false;
  ...
  private final boolean isSeenState(final TLCState curState,
                                    final TLCState succState,
                                    final Action action, final Worker worker,
                                    final SetOfStates liveNextStates)
      throws IOException {
    final long fp = succState.fingerPrint();
    final boolean seen = this.theFPSet.put(fp);
    // Write out succState when needed:
    this.allStateWriter.writeState(
        curState, succState, seen ? IStateWriter.IsSeen : IStateWriter.IsUnseen,
        action);
    if (!seen) {
  ...
  private final boolean doNextCheckInvariants(final ITool tool,
                                              final TLCState curState,
                                              final TLCState succState)
  ...
  private final boolean doNextCheckImplied(final ITool tool,
                                           final TLCState curState,
                                           final TLCState succState)
      throws IOException, WorkerException, Exception {
    int k = 0;
    try {
      for (k = 0; k < tool.getImpliedActions().length; k++) {
        if (!tool.isValid(tool.getImpliedActions()[k], curState, succState)) {
          ...
          if (TLCGlobals.continuation) {
            synchronized (this) {
              MP.printError(EC.TLC_ACTION_PROPERTY_VIOLATED_BEHAVIOR,
      ...
      doNextEvalFailed(curState, succState,
                       EC.TLC_ACTION_PROPERTY_EVALUATION_FAILED,
                       tool.getImpliedActNames()[k], e);
    }
    return false;
  }

  final boolean doNextSetErr(TLCState curState, TLCState succState,
                             boolean keep, int ec, String param)
  ...
  final void doNextEvalFailed(TLCState curState, TLCState succState, int ec,
                              String param, Exception e)
      throws IOException, WorkerException, Exception {
    synchronized (this) {
      if (this.setErrState(curState, succState, true, ec)) {
        MP.printError(ec, new String[] {param, (e.getMessage() == null)
                                                   ? e.toString()
                                                   : e.getMessage()});
        this.trace.printTrace(curState, succState);
        this.theStateQueue.finishAll();
  ...
  final void doNextFailed(TLCState curState, TLCState succState, Throwable e)
    ...
    synchronized (this) {
      ...
      if (e instanceof StackOverflowError) {
        ...
        // TODO: 345hv87 Read errorCode from TLCRE into ec. However, too much
        // legacy code reports TLCRE errors as EC.General, and, thus, this
        // change has the risk of causing regression (it causes test failures in
        // e.g.: ETest6, FingerprintExceptionNextTest, tlc2.tool.Github611Test)
        //			} else if (e instanceof TLCRuntimeException)
        //			{
        //				ec = ((TLCRuntimeException)
        //e).errorCode;
      } else {
        ec = EC.GENERAL;
      }
      ...
      if (this.setErrState(curState, succState, !keep, ec)) {
        if (!(ec == EC.GENERAL && e.getMessage() == null)) {
          if (e instanceof EvalException &&
              ((EvalException)e).hasParameters()) {
            // An EvalException pretty-prints itself in its constructor, i.e.
            // converts the parameters into the human readable string. However,
            // MP.print* will pretty-print it a second time, which is why we
            // pass the original parameters instead of the EvalException itself.
            // Exception handling in TLC is a mess!
            MP.printError(ec, ((EvalException)e).getParameters(), e);
            // TODO: See note above at label "345hv87".
            //					} else if (e instanceof
            //TLCRuntimeException) { 						MP.printError(ec, ((TLCRuntimeException)
            //e).parameters);
          } else {
            MP.printError(ec, e);
          }
  ...
  public final int doPeriodicWork() throws Exception {
    ...
    if ((!this.checkLiveness || runtimeRatio > TLCGlobals.livenessRatio ||
         !liveCheck.doLiveCheck()) &&
        !forceLiveCheck && !createCheckPoint && periodic == null) {
    ...
    if (this.theStateQueue.suspendAll()) {
      ...
      if (this.checkLiveness &&
          (runtimeRatio < TLCGlobals.livenessRatio || forceLiveCheck)) {
      ...
      if (periodic != null &&
          BoolValue.ValFalse.equals(tool.noDebug().eval(periodic))) {
  ...
  protected void checkpoint() throws IOException {
    ...
    // ---- ATOMIC COMMIT MANIFEST WAL ----
    // We construct a Write-Ahead Log (WAL) marker to indicate that a checkpoint
    // commit is in progress. This marker (`commit.inprogress`) is crucial for
    // detecting split-brain scenarios where a hard crash occurs midway through
    // renaming `.tmp` files to `.chkpt` files. During recovery, the existence
    // of this marker proves that the checkpoint was torn, allowing us to safely
    // rollback to the `.chkpt.bak` files created in the previous epoch.
    {

      java.io.File metaDirF = new java.io.File(this.metadir);
      java.io.File commitInprogress =
          new java.io.File(metaDirF, "commit.inprogress");

      // Backup all current .chkpt files
      java.io.File[] files = metaDirF.listFiles();
      if (files != null) {
        for (int i = 0; i < files.length; i++) {
          java.io.File f = files[i];
          if (f.getName().endsWith(".chkpt")) {
            java.io.File bakFile =
                new java.io.File(metaDirF, f.getName() + ".bak");
            if (bakFile.exists())
              bakFile.delete();
            java.nio.file.Files.createLink(bakFile.toPath(), f.toPath());
          }
        }
      }

      // Write the WAL marker and fsync it!
      util.FileUtil.SyncingFileOutputStream markerFos =
          new util.FileUtil.SyncingFileOutputStream(commitInprogress);
      markerFos.write(1);
      markerFos.close();

      // Ensure WAL marker is durable before proceeding
      util.FileUtil.syncDirectory(this.metadir);
      ...
      // Resume workers immediately after InternTable and queues have safely
      // serialized their graphs, but BEFORE we execute the heavy synchronous
      // disk I/O (FileUtil.syncDirectory) and marker deletions. Resuming here
      // strictly prevents String ID collisions while maintaining the strict BFS
      // timing required for trace determinism.
      this.theStateQueue.resumeAll();

      // Ensure renames are durable before deleting marker
      // By enforcing an fsync on the directory descriptor, we guarantee that
      // all rename operations (e.g. queue.tmp -> queue.chkpt) have been flushed
      // from the OS buffer cache to physical media. This strict ordering
      // ensures that we only delete the `commit.inprogress` marker AFTER the
      // new checkpoint is completely and durably assembled on disk, preventing
      // the torn-checkpoint vulnerability.
      util.FileUtil.syncDirectory(this.metadir);

      // Delete marker
      commitInprogress.delete();

      // Ensure marker deletion is durable before deleting backups
      util.FileUtil.syncDirectory(this.metadir);

      // Delete backups
      files = metaDirF.listFiles();
      if (files != null) {
        for (int i = 0; i < files.length; i++) {
          java.io.File f = files[i];
          if (f.getName().endsWith(".chkpt.bak")) {
            f.delete();
          }
        }
      }

      // After marker is deleted, vacuum old state pools
      // We exclusively invoke pool cleanup here, rather than allowing a
      // detached background thread to clean up files speculatively. This
      // guarantees that `DiskStateQueue` and `DiskByteArrayQueue` never delete
      // historical pool files until the exact moment those files are
      // definitively superseded by the newly committed checkpoint manifest,
      // thereby eliminating the "No such file or directory" crash on recovery.
      this.theStateQueue.vacuum();
  ...
  public void forceLiveCheck() { forceLiveCheck = true; }
  ...
  protected void updateRuntimeRatio(final long delta) {
    ...
    // new ratio.
    runtimeRatio = (delta + absLivenessRuntime) /
                   (totalRuntime + TLCGlobals.progressInterval + delta);
  }

  public double getRuntimeRatio() { return runtimeRatio; }

  public final boolean recover() throws IOException {
    boolean recovered = false;
    if (this.fromChkpt != null) {
      // ---- ATOMIC RECOVERY WAL ----
      // Before reading any checkpoint files, we must scan for the
      // `commit.inprogress` marker. If present, it serves as an
      // incontrovertible proof that the previous process was violently killed
      // during the critical section of a checkpoint commit. The atomic `.chkpt`
      // files on disk are therefore untrustworthy (torn state). We must
      // rigorously revert every `.chkpt` file back to its corresponding
      // `.chkpt.bak` file from the previous stable epoch to restore
      // deterministic consistency.
      java.io.File metaDirF = new java.io.File(this.metadir);
      java.io.File commitInprogress =
          new java.io.File(metaDirF, "commit.inprogress");
      if (commitInprogress.exists()) {
        // We suffered a hard crash mid-commit. Revert split-brain!
        java.io.File[] files = metaDirF.listFiles();
        if (files != null) {
          // Handle orphaned .chkpt files that have no .bak backup (e.g. crash
          // on first checkpoint)
          for (int i = 0; i < files.length; i++) {
            java.io.File f = files[i];
            if (f.getName().endsWith(".chkpt")) {
          ...
          for (int i = 0; i < files.length; i++) {
            ...
            if (f.getName().endsWith(".chkpt.bak")) {
              ...
              java.nio.file.Files.copy(
                  f.toPath(), new java.io.File(metaDirF, origName).toPath(),
      ...
      // Check if essential checkpoint files actually exist before attempting
      // recovery
      if (!new java.io.File(metaDirF, "queue.chkpt").exists()) {
        MP.printMessage(
            EC.GENERAL,
            "Checkpoint is incomplete or missing. Starting from scratch.");
        return false;
      }
      ...
      MP.printMessage(EC.TLC_CHECKPOINT_RECOVER_END,
                      new String[] {String.valueOf(this.theFPSet.size()),
  ...
  private final void cleanup(boolean success) throws IOException {
    ...
    // either and error has been found or time-bound model checking is enabled,
    // take a snapshot to allow users to continue model checking if needed.
    if (TLCGlobals.chkptExplicitlyEnabled() && !theStateQueue.isEmpty() &&
        (this.errState != null || isTimeBound())) {
      checkpoint();
      vetoCleanup = true;
    }
  ...
  public final void printSummary(boolean success, final long startTime)
      throws IOException {
    ...
    MP.printMessage(EC.TLC_STATS,
                    new String[] {String.valueOf(getStatesGenerated()),
                                  String.valueOf(this.theFPSet.size()),
                                  String.valueOf(this.theStateQueue.size())});
    // The depth used to only be reported on success, but this seems bogus since
    // TLC reports the number states above.
    MP.printMessage(EC.TLC_SEARCH_DEPTH,
                    String.valueOf(getStatesGenerated() == 0L
                                       ? 0
                                       : this.trace.getLevelForReporting()));
    if (success) {

      // Aggregate outdegree from statistics maintained by individual workers.
      final BucketStatistics aggOutDegree =
          new BucketStatistics("State Graph OutDegree");
  ...
  private final void printProgresStats(final long startTime,
    ...
    MP.printMessage(
        EC.TLC_PROGRESS_STATS,
        new String[] {
            String.valueOf(this.trace.getLevelForReporting()), MP.format(l),
            MP.format(fpSetSize), MP.format(this.theStateQueue.size()),
            MP.format(statesPerMinute), MP.format(distinctStatesPerMinute)});
  }

  public static final void reportSuccess(final FPSet anFpSet,
                                         final long numOfGenStates)
      throws IOException {
    final long numOfDistinctStates = anFpSet.size();
    final double optimisticProb =
        calculateOptimisticProbability(numOfDistinctStates, numOfGenStates);
   ...
   * Process calculation.
   *
   * Comments added 9 April 2012 by LL.  The above was Simon's extensive
   * commenting.  I presume he really mean "ProGRess calculation", since this
   * seems to be where the coverage and progress information is written.  The
   * method writes the progress information, prints the coverage only if count =
   * 0, and then waits until it's time to print the next progress report before
   * exiting.  (The next progress report is printed the next time the method is
   * called.)
   *
   ...
   * @throws Exception
   */
  protected void runTLCContinueDoing(final int count, final int depth)
      throws Exception {
   ...
   * @param e
   */
  private void report(Throwable e) { DebugPrinter.print(e); }
  ...
  @Override
  public TLCStateInfo[] getTraceInfo(final TLCState from, TLCState to)
      throws IOException {
  ...
  private class DoInitFunctor implements IStateFunctor {
    ...
    public DoInitFunctor(ITool tool) { this(tool, false); }
    ...
    public Object addElement(final TLCState curState) {
      if (Long.bitCount(numberOfInitialStates) == 1 &&
          numberOfInitialStates > 1) {
        MP.printMessage(EC.TLC_COMPUTING_INIT_PROGRESS,
                        Long.toString(numberOfInitialStates));
      }
      ...
      try {
        ...
        if (!tool.isGoodState(curState)) {
          MP.printError(EC.TLC_INITIAL_STATE,
                        new String[] {"current state is not a legal state",
                                      curState.toString()});
        ...
        if (!seen || forceChecks) {
          for (int j = 0; j < tool.getInvariants().length; j++) {
            if (!tool.isValid(tool.getInvariants()[j], curState)) {
              ...
              MP.printError(
                  ...
                  new String[] {tool.getInvNames()[j].toString(),
          ...
          for (int j = 0; j < tool.getImpliedInits().length; j++) {
            if (!tool.isValid(tool.getImpliedInits()[j], curState)) {
              ...
              MP.printError(
                  EC.TLC_PROPERTY_VIOLATED_INITIAL,
                  new String[] {tool.getImpliedInitNames()[j],
                                tool.evalAlias(curState, curState).toString()});
              this.errState = curState;
      ...
      } catch (InvariantViolatedException | Assert.TLCRuntimeException |
               EvalException e) {
        // IVE gets thrown above when an Invariant is violated.
        // TLCRuntimeException gets thrown when Tool fails to evaluate a
        // statement because of e.g. too large sets or type errors such as in
        // DoInitFunctorInvariantMinimalErrorStackTest test.
        this.errState = curState;
        this.e = e;
        throw e;
