 ...
 *
 * - Logically stores a set of triples that represent the liveness/behavior
 * graph (see Manna/Pnuelli book).
 * - Technically, it stores the triple of <<state (fingerprint), tableau node
 * index, link (transitions)>>
 ...
 *     GraphNode#getTNode(TBGraph). One obviously has to have access to the
 * TBGraph
 * -- Link(s) are kept in GraphNode#nnodes
 *
 * - {@link DiskGraph#toString()} does not print init nodes. They never get
 *   written to the {@link BufferedRandomAccessFile}s {@link
 * DiskGraph#nodePtrRAF} & {@link DiskGraph#nodeRAF}
 * - {@link DiskGraph#toString()} only prints the part of the DiskGraph that is
 * on disk. It ignores the in-memory part. This means toString produces *no*
 * output for as long as the graph has *not* been flushed to disk
 *
 * - On disk, the {@link BufferedRandomAccessFile}s are suffixed by the ID of
 * the
 *   {@link DiskGraph} (we can have >1 when there are more {@link
 * OrderOfSolution})
 */
public abstract class AbstractDiskGraph {
  ...
  public AbstractDiskGraph(String metadir, int soln,
                           IBucketStatistics graphStats) throws IOException {
  ...
  public long addNode(GraphNode node) throws IOException {
    ...
    // TODO DiskGraph without a tableau don't need the tindex. The only reason
    // it is written to disk, is to use the same implementation for trace path
  ...
  public abstract GraphNode getNode(long fingerprint, int tableauIdx)
      throws IOException;
  ...
  public synchronized final GraphNode getNode(final long stateFP,
                                              final int tidx, final long ptr)
      throws IOException {
  ...
  protected synchronized final GraphNode getNodeFromDisk(final long stateFP,
                                                         final int tidx,
                                                         final long ptr)
      throws IOException {
    ...
    if (ptr < 0) {
      throw new IllegalArgumentException("Invalid negative file pointer: " +
                                         ptr);
    }
  ...
  public LongVec getPath(final long state, final int tidx) throws IOException {
    throw new RuntimeException(
        "Couldn't re-create liveness trace (path) starting at: " + state +
        " and tidx: " + tidx);
  }
  ...
  public long getSizeAtLastCheck() { return sizeAtCheck; }

  public void recordSize() { this.sizeAtCheck = size(); }
  ...
  protected String toDotVizLegend(final OrderOfSolution oos) {
    ...
    for (LiveExprNode liveExprNode : checkState) {
      sb.append(String.format("S%s [label=\"S%s: %s\"]", i, i++,
                              node2dot(liveExprNode)));
      sb.append("\n");
    }
    // Actions checks
    i = 1;
    ...
    for (LiveExprNode liveExprNode : checkState) {
      sb.append(String.format("A%s [label=\"A%s: %s\"]", i, i++,
                              node2dot(liveExprNode)));
      sb.append("\n");
    }
  ...
  protected static String node2dot(final LiveExprNode node) {
    // Replace "\" with "\\" and """ with "\"".	Replace "<" and ">" with "\<"
    // and "\>".
  ...
  public synchronized final void beginChkpt() throws IOException {
    this.flushWritesToDiskFiles();
    util.FileUtil.SyncingFileOutputStream fos =
        new util.FileUtil.SyncingFileOutputStream(this.chkptName +
                                                  ".chkpt.tmp");
  ...
  public final void recover() throws IOException {
    String filename = chkptName + ".chkpt";
    try {
   ...
   *
   * <p>This method has essentially no visible effect, since any disk files are
   * private to this object and flushing writes should not affect reads from
   * those files.  However, there are some tests that call this method to make
  ...
  // This method is not called anywhere because *out degree* graph statistics
  // are collected during liveness checking with negligible overhead (see
  // DiskGraph#addNode).
  public void
  calculateOutDegreeDiskGraph(final IBucketStatistics outDegreeGraphStats)
      throws IOException {
    try {
      this.nodePtrRAF.flush();
      this.nodeRAF.flush();
  ...
  public void
  calculateInDegreeDiskGraph(final IBucketStatistics inDegreeGraphStats)
      throws IOException {
    ...
    final Map<NodeRAFRecord, Integer> nodes2count =
        new HashMap<NodeRAFRecord, Integer>();
    ...
    // contains vertices that are successors of a vertex stored in the
    // nodePtrRAF. The nodePtrRAF is the primary disk storage file with a
    // fingerprint & tableau id and a pointer to the successor nodes in nodeRAF.
    // While a node appears only once in the nodePtrRAF, the same node is
    // potentially listed in nodeRAF multiple times.
    try {
      this.nodeRAF.flush();
      ...
      while (this.nodeRAF.getFilePointer() < len) {
        ...
        nodeRAF.seek(nodeRAF.getFilePointer() +
                     (checksLen * 8)); // 8 bytes is long
      }
    } catch (IOException e) {
      MP.printError(EC.SYSTEM_DISKGRAPH_ACCESS, e);
      System.exit(1);
  ...
  private class NodeRAFRecord {
    ...
    private AbstractDiskGraph getOuterType() { return AbstractDiskGraph.this; }
  }
}
