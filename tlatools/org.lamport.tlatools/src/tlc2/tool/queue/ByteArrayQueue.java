...
public abstract class ByteArrayQueue implements IStateQueue {
  ...
  private final byte[] toBytes(final TLCState state) {
    try {
      final DiskByteArrayQueue.ByteValueOutputStream vos =
          new DiskByteArrayQueue.ByteValueOutputStream();
      state.write(vos);
  ...
  private final boolean isAvail() {
    ...
    while (isEmpty() || this.stop) {
      ...
      if (this.numWaiting >= TLCGlobals.getNumWorkers()) {
        ...
        // TLCServer.
        synchronized (this.mu) { this.mu.notify(); }
      }
      try {
        this.wait();
      } catch (Exception e) {
        MP.printError(EC.GENERAL,
                      "making a worker wait for a state from the queue",
                      e); // LL changed call 7 April 2012
        System.exit(1);
      }
      this.numWaiting--;
      if (this.finish) {
    ...
    while (needWait) {
      synchronized (this.mu) {
        try {
          ...
          MP.printError(EC.GENERAL, "waiting for a worker to wake up",
                        e); // LL changed call 7 April 2012
          System.exit(1);
        }
      }
