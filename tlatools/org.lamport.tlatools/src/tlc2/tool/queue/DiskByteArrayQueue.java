 ...
 * states. An in-memory buffer of size {@link DiskByteArrayQueue}{@link
 * #BufSize}
 */
public class DiskByteArrayQueue extends ByteArrayQueue {
  ...
  public final void vacuum() throws IOException {
    ...
    // the aggressive `fillDeqBuffer()` loop, we mathematically ensure that no
    // pool file is ever deleted while it remains referenced by the surviving
    // `queue.chkpt` manifest on disk. This architectural isolation is required
    // to prevent fatal "No such file or directory" failures during crash
    // recovery.
    final int start;
    final int end;
    synchronized (this) {
  ...
  private static final class ByteArrayPoolWriter extends Thread {
    ...
    public final synchronized byte[][] doWork(byte[][] enqBuf, File file)
      ...
      if (this.poolFile != null) {
        final BufferedDataOutputStream vos =
            new BufferedDataOutputStream(this.poolFile);
  ...
  private static final class ByteArrayPoolReader extends Thread {
    ...
    public final synchronized byte[][] doWork(byte[][] deqBuf, File file)
      ...
      if (this.isFull) {
        ...
        final BufferedDataInputStream vis =
            new BufferedDataInputStream(this.poolFile);
  ...
  static final class ByteValueOutputStream
      implements IValueOutputStream, IDataOutputStream {
  ...
  static final class ByteValueInputStream
      implements ValueConstants, IValueInputStream, IDataInputStream {

    private final byte[] bytes;

    private int idx = 0;

    public ByteValueInputStream(byte[] bytes) { this.bytes = bytes; }
    ...
    public final IValue read() throws IOException {
      ...
      default: {
        throw new WrongInvocationException(
            "ValueInputStream: Can not unpickle a value of kind " + kind);
      }
      }
    }
