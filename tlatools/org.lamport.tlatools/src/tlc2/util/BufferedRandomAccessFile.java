 ...
 *         really been written (the same way that {@link
 * java.io.RandomAccessFile#write(byte[])} would write them).</li> <li>{@link
 * #invalidateBufferedData()} ensures that all previously-performed writes
 * through other file descriptors are now readable through this one.</li>
 * </ul>
 *
 * @author Allan Heydon
 */
public final class BufferedRandomAccessFile extends java.io.RandomAccessFile {
  ...
  /**
   * A bitwise mask composed of leading ones followed by exactly {@link
   * #LogBuffSz} zeros.  It can be used to quickly round down to the nearest
   * {@link #BuffSz}-aligned offset: <pre> offset - (offset % BuffSz) == offset
   * & BuffMask
   * </pre>
   */
  // NOTE: some bitwise trickery going on here!  Because BuffSz is a positive
  // power of 2 and ints are represented in twos-complement, simply negating
  // BuffSz produces the mask we seek.  The actual integer value of the result
  // is irrelevant; we only care about the bits here.
  ...
  /**
   * Cached value of <code>super.{@link #getFilePointer()}</code>.  Used to
   * avoid some calls to <code>super.{@link #seek(long)}</code>.
   */
  ...
  */
   ...
   * and writing.
   *
   * @deprecated Avoid stringly-typed code; use {@link
   *     #BufferedRandomAccessFile(File, String)} instead.
   */
  @Deprecated
   ...
   * Clients will typically call this when they know that the on-disk contents
   * have been altered through a different file descriptor and they need to
   * observe those writes through this one.
   *
   * <p>If this object has any buffered writes, this call will flush them to
   * disk ({@link #flush()}).
   *
  ...
  public void invalidateBufferedData() throws IOException {
    ...
    // To preserve this class's invariants, we can't just clear the buffer; we
    // also need to refill the buffer from disk.
    fillBuffer();
  }
  ...
  private void fillBuffer() throws IOException {
    ...
    // 1. We didn't fill the buffer because we hit EOF (i.e. length < lo +
    // BuffSz).
    // 2. The client calls seek() to jump forward, followed by write() to write
    // a byte later in the buffer.
    //
    // Then the unset part of buff (represented by "*" below) can actually
    // matter:
    //
    ...
    //            seek(7)      write(0xFF)
    //
    // In that case, the file must be extended to contain the write, and the
    // unset part of the buffer becomes part of the official file contents.
    // Fortunately the RandomAccessFile contract gives us an out: when the file
    // is extended in this way, the unwritten portion can be filled with
    // arbitrary data.  Therefore, we do not need to do anything to the unset
    // parts of buff.
    //
    ...
    // OFFERING A STRONGER CONTRACT!  There are still cases where we rely on
    // "super" to extend the file---meaning we inherit the weakest possible
    // contract from RandomAccessFile.  In particular, this happens when
    // extending the file using setLength() or when the client makes a giant
    // forward seek followed by a write.
  }
   ...
   *
   * <p>This method behaves exactly like {@link
   * java.io.RandomAccessFile#setLength(long)}, but fully constrains the
   * behavior of the file pointer: after this call, {@link #getFilePointer()}
   * will be the minimum of the old file pointer and the new file length.
   *
  ...
  public void setLength(long newLength) throws IOException {
    ...
    //
    // In practice, RandomAccessFile actually moves the pointer if
    // getFilePointer > newLength, regardless of whether the file was truncated
    // or extended.
    super.setLength(newLength);
    this.length = newLength;

    // In theory, we should only have to update `this.diskPos` when
    // `this.diskPos > newLength` (see note above). But, because the
    // RandomAccessFile docs are vague on this point, let's be careful.
    this.diskPos = super.getFilePointer();

    // Because the RandomAccessFile docs are vague, we have a bit of freedom
    // here (see note above).  But let's do exactly what we observed in practice
    // (Java 11 and 21, April 2024): set `curr` to `min(curr, newLength)`.
    //
    // Note that there are two reasons why the obvious-looking call
    // `seek(diskPos)` would be incorrect:
    //  1. We would inherit the underspecification mentioned above regarding the
    //  file pointer.  We have an
    //     opportunity to refine that specification with something more precise.
    //  2. Since `diskPos` was not necessarily in sync with `curr` at the start
    //  of this method, it is not likely
  ...
  /**
   * Restore relevant invariants (in particular V2) after increasing {@link
   * #curr}.
   *
  ...
  private void restoreInvariantsAfterIncreasingCurr() throws IOException {
    ...
    // inlining `seeek` and removing redundant checks and code.
  ...
  public int read() throws IOException {
    ...
    // Read the next byte out of the cache.  Invariant V2 guarantees that this
    // read is in-bounds.
  ...
  public int read(byte[] b, int off, int len) throws IOException {
    ...
    long numReadableWithoutSeeking =
        Math.min(this.lo + BuffSz, this.length) - this.curr;
  ...
  public void write(int b) throws IOException {
    ...
    // Write the byte into the buffer.  Invariant V2 guarantees that this write
    // is in-bounds.
  ...
  public void write(byte[] b, int off, int len) throws IOException {
    ...
    assert super.getFilePointer() == diskPos;
    while (len > 0) {
      // NOTE: this implementation might be a little wasteful!  Because
      // `writeAtMost` has to maintain invariant V2, it will actually do reads
      // from disk when the write crosses outside the current buffer.  This
      // could be improved by relaxing V2 during this loop (or potentially
      // removing that invariant entirely).
  ...
  private int writeAtMost(byte[] b, int off, int len) throws IOException {
    int numWriteableWithoutSeeking =
        Math.min(len, (int)(this.lo + BuffSz - this.curr));
   ...
   * file. The previous content of the underlying disk file will be overwritten
   * and the file pointer will be moved to the beginning of the file.
   *
   * @throws IOException
   */
